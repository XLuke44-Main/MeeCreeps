...
public class PortalTESR extends TileEntitySpecialRenderer<PortalTileEntity> {
    ...
    private static final ResourceLocation portalBlue = new ResourceLocation(MeeCreeps.MODID, "textures/effects/portal_blue.png");
    ...
    public void render(PortalTileEntity te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        ...
        this.bindTexture(te.isBlueFluid() ? portalBlue : portal);

        // The animated UVs used below intentionally extend slightly outside [0, 1] so
        // the portal can rotate without changing its size. These textures have a
        // transparent border, so they must clamp to that border instead of repeating
        // the opposite edge. With GL_REPEAT the out-of-range samples wrap around and
        // create a hard vertical/horizontal seam that moves with the animation angle.
    ...
    public static void renderQuadBright(double angle, Face face, Face rface) {
        ...
        // The portal texture has a circular alpha mask. The animated UVs form a
        // rotated square; with f=1.75 its inscribed radius is ~0.404, smaller
        // than the texture's ~0.451 non-transparent radius, so the square clips
        // the circle and creates straight edge cuts as the UVs rotate.
        double f = 1.5;
