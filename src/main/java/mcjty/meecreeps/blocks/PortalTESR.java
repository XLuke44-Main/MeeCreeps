...
public class PortalTESR extends TileEntitySpecialRenderer<PortalTileEntity> {
    ...
    private static final double PORTAL_FACE_EPSILON = 0.001D;
    ...
    public void render(PortalTileEntity te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        ...
        if (te.getPortalSide() == null || te.getSpawnerPos() != null) {
            return;
        }
        ...
        renderOx += te.getPortalSide().getFrontOffsetX() * PORTAL_FACE_EPSILON;
        renderOy += te.getPortalSide().getFrontOffsetY() * PORTAL_FACE_EPSILON;
        renderOz += te.getPortalSide().getFrontOffsetZ() * PORTAL_FACE_EPSILON;
    ...
    public static void renderSpawnerPortal(PortalTileEntity te, double x, double y, double z) {
        if (te == null || te.getPortalSide() == null || te.getSpawnerPos() == null) {
            return;
        }
        ...
        try {
            ...
            Minecraft.getMinecraft().getTextureManager().bindTexture(portalTexture);
            GlStateManager.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_WRAP_S, GL12.GL_CLAMP_TO_EDGE);
            GlStateManager.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_WRAP_T, GL12.GL_CLAMP_TO_EDGE);
            GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
            ...
            GlStateManager.enablePolygonOffset();
            GlStateManager.doPolygonOffset(-1.0F, -4.0F);
            try {
                renderQuadBright(angle, face, rface);
            } finally {
                GlStateManager.disablePolygonOffset();
