...
public class PortalSpawnerBlock extends Block {
    ...
    private boolean tryFindAndLinkExistingTwin(World currentWorld, BlockPos currentUpperPos,
        ...
        PortalSpawnerTileEntity candidate = PortalSpawnerRegistry.find(pairId, currentTe);
        if (candidate == null) {
            return false;
        }

        World candidateWorld = candidate.getWorld();
        if (candidateWorld == null || candidate.getPos().equals(currentUpperPos)) {
            return false;
        }
        ...
        currentTe.setTwin(candidateWorld.provider.getDimension(), candidate.getPos());
