...
public class PortalTileEntity extends TileEntity implements ITickable {
    ...
    public void update() {
        ...
        try {
            ...
            if (blueFluid && other.getDimension() != world.provider.getDimension()) {
                killPortal();
                return;
            }
            ...
            // Keep the arrival latch until the entity leaves the destination portal. A separate
            // short cooldown prevents a same-tick source->destination->source bounce even when
            // two differently oriented portal trigger boxes overlap the arrival point.
            ...
            for (Entity entity : entities) {
                ...
                // Automatic post-entry extension must never undo an explicit J/manual close.
            ...
            try { killPortal(); } catch (Throwable ignored) { }
        }
    }
    ...
    @Override public SPacketUpdateTileEntity getUpdatePacket() {
        ...
        nbtTag.setBoolean("blueFluid", blueFluid);
    ...
    @Override public void onDataPacket(NetworkManager net, SPacketUpdateTileEntity packet) {
        ...
        blueFluid = packet.getNbtCompound().getBoolean("blueFluid");
    }
    ...
    private AxisAlignedBB getTeleportBox() {
        if (box == null) {
            ...
            // The portal is rendered as a 2x2 plane. Keep the trigger close to that
            // plane instead of using a large padded volume around it. This prevents
            // teleporting while the player is still several blocks away, while still
            // allowing normal entity bounding-box intersection as the player passes
            // through the portal.
    ...
    public void forceManualClose() {
        // Always refresh this endpoint's closing timer, even when another close request has
        // already reached it. Only propagation is guarded, preventing recursive A -> B -> A
        // calls while guaranteeing that both endpoints enter the closing animation.
    ...
    public boolean isBlueFluid() { return blueFluid; }
    public void setBlueFluid(boolean blueFluid) { this.blueFluid = blueFluid; markDirtyClient(); }
    public void setAutoClose(boolean autoClose) { this.autoClose = autoClose; markDirtyClient(); }
    public void killPortal() {
        if (world != null && !world.isRemote) {
            if (chunkTicket != null) {
                try { ForgeChunkManager.releaseTicket(chunkTicket); } catch (Throwable ignored) { }
                chunkTicket = null;
            }
            if (world.getBlockState(getPos()).getBlock() == ModBlocks.portalBlock) world.setBlockToAir(getPos());
        }
    }
    ...
    @Override public void readFromNBT(NBTTagCompound compound) {
        ...
        blueFluid = compound.getBoolean("blueFluid");
        ...
        // blackListed is deliberately runtime-only arrival protection. Do not restore it from world NBT.
        blackListed.clear();
    }
    ...
    @Override public NBTTagCompound writeToNBT(NBTTagCompound compound) {
        ...
        compound.setBoolean("blueFluid", blueFluid);
        compound.setBoolean("manualCloseRequested", manualCloseRequested);
        if (gunId != null) compound.setUniqueId("gunId", gunId);
        // blackListed is intentionally not serialized. It only prevents immediate re-entry after teleport.
