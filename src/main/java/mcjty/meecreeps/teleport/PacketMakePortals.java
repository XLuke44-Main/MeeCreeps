...
public class PacketMakePortals implements IMessage {
    ...
    @Override public void fromBytes(ByteBuf buf) {
        try {
            ...
            validPacket = true;
        } catch (Exception t) { validPacket = false; }
    }
    ...
    public void handle(Supplier<Context> supplier) {
        ...
        ctx.enqueueWork(() -> {
            ...
            try {
                ...
                if (!TeleportationTools.isWithinInteractionRange(player, selectedBlock)
                        || !player.canPlayerEdit(selectedBlock, selectedSide, heldItem)) return;
