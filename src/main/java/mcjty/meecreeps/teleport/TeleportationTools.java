...
public class TeleportationTools {
    ...
    public static void cancelPortalPair(EntityPlayer player, BlockPos selectedBlock) {
        try {
            ...
            if (dx * dx + dy * dy + dz * dz > 64.0D) return;
            ...
            if (te instanceof PortalTileEntity) {
                ...
                // Portals created by the player-owned portal gun are tied to that gun.
                // Server-side permission must be checked here because the client supplies the
                // portal position in the command packet. Untagged API-created portals remain
                // manually closable for backwards-compatible behavior.
    ...
    private static boolean createPair(World sourceWorld, BlockPos sourcePortalPos, EnumFacing sourceSide, World destWorld, TeleportDestination destination, java.util.UUID gunId, boolean autoClose) {
        return createPair(sourceWorld, sourcePortalPos, sourceSide, destWorld, destination, gunId, autoClose, false, TransportSolutionType.INTERDIMENSIONAL);
    }
