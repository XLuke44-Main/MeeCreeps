...
public class TeleportationTools {
    ...
    public static Entity teleportEntity(Entity entity, World destWorld, double newX, double newY, double newZ,
        ...
        Vec3d transformed = transformLook(look, sourceFacing, destinationFacing);
        float yaw = (float) (Math.atan2(-transformed.x, transformed.z) * 180.0D / Math.PI);
        float pitch = (float) (-Math.asin(Math.max(-1.0D, Math.min(1.0D, transformed.y))) * 180.0D / Math.PI);
    ...
    private static Vec3d transformLook(Vec3d look, EnumFacing sourceFacing, EnumFacing destinationFacing) {
        ...
        double right = look.dotProduct(sourceRight);
        double up = look.dotProduct(sourceUp);
        double normal = look.dotProduct(sourceNormal);
