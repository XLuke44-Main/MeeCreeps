...
public class TeleportationTools {
    ...
    public static void cancelPortalPair(EntityPlayer player, BlockPos selectedBlock) {
        try {
            ...
            if (te instanceof PortalTileEntity) {
                // A manual close is a pair operation. Setting only this endpoint's timeout
                // allows the other endpoint to disappear without ever entering its closing
                // animation, especially when the partner is in another dimension.
                ((PortalTileEntity) te).forceManualClose();
            }
    ...
    public static void makePortalPair(EntityPlayer player, BlockPos selectedBlock, EnumFacing selectedSide, TeleportDestination dest, java.util.UUID gunId, boolean autoClose) {
        ...
        makePortalPair(player, selectedBlock, selectedSide, dest, gunId, autoClose,
                !heldGun.isEmpty() && PortalGunItem.isBlueFluid(heldGun));
    }

    private static void makePortalPair(EntityPlayer player, BlockPos selectedBlock, EnumFacing selectedSide, TeleportDestination dest, java.util.UUID gunId, boolean autoClose, boolean blueFluid) {
        makePortalPair(player, selectedBlock, selectedSide, dest, gunId, autoClose, null, false, blueFluid);
    }
    ...
    public static boolean makeAirPortalPair(EntityPlayer player, BlockPos sourcePortalPos, EnumFacing sourceSide, TeleportDestination dest, java.util.UUID gunId, boolean autoClose) {
        net.minecraft.item.ItemStack heldGun = PortalGunItem.getGun(player);
        return makePortalPair(player, null, sourceSide, dest, gunId, autoClose, sourcePortalPos, true,
                !heldGun.isEmpty() && PortalGunItem.isBlueFluid(heldGun));
    }

    private static boolean makePortalPair(EntityPlayer player, BlockPos selectedBlock, EnumFacing selectedSide, TeleportDestination dest, java.util.UUID gunId, boolean autoClose, @Nullable BlockPos sourcePortalPosOverride, boolean allowAirSource, boolean blueFluid) {
        try {
            ...
            if (!heldGun.isEmpty() && !PortalGunItem.isDestinationAllowed(heldGun, player.getEntityWorld(), dest)) {
            ...
            // The source/origin portal always uses the original placement logic unless the
            // explicit air-placement mode supplied an already-selected air position.
            ...
            if (destWorld.getBlockState(dest.getPos()).getBlock() == ModBlocks.portalBlock) {
                showBalloon(player, TextFormatting.WHITE + I18n.format("message.meecreeps.portal_already_there"));
                return false;
            }
            ...
            boolean created = createPair(sourceWorld, sourcePortalPos, selectedSide, destWorld, finalDestination, gunId, autoClose, allowAirSource, blueFluid);
            if (created) {
    ...
    private static ItemStackHolder getPlayerGunState(EntityPlayer player) {
        if (player == null) return new ItemStackHolder(null, true, false);
        net.minecraft.item.ItemStack gun = PortalGunItem.getGun(player);
        if (gun.isEmpty()) return new ItemStackHolder(null, true, false);
        return new ItemStackHolder(PortalGunItem.getGunId(gun), PortalGunItem.isAutoCloseEnabled(gun), PortalGunItem.isBlueFluidActive(gun));
    }

    private static class ItemStackHolder {
        private final java.util.UUID gunId;
        private final boolean autoClose;
        private final boolean blueFluid;
        private ItemStackHolder(java.util.UUID gunId, boolean autoClose, boolean blueFluid) { this.gunId = gunId; this.autoClose = autoClose; this.blueFluid = blueFluid; }
    }
    ...
    public static void makePortalPair(World sourceWorld, BlockPos selectedBlock, EnumFacing selectedSide, TeleportDestination dest) {
        makePortalPair(sourceWorld, selectedBlock, selectedSide, dest, false);
    }

    public static void makePortalPair(World sourceWorld, BlockPos selectedBlock, EnumFacing selectedSide, TeleportDestination dest, boolean blueFluid) {
        try {
            if (blueFluid && (dest == null || dest.getDimension() != sourceWorld.provider.getDimension())) return;
            ...
            TeleportDestination finalDestination = new TeleportDestination(dest.getName(), dest.getDimension(), safeDestination.pos, safeDestination.side, dest.shouldSearchNearby());
            createPair(sourceWorld, sourcePortalPos, selectedSide, destWorld, finalDestination, null, true, false, blueFluid);
        } catch (Throwable ignored) {
        }
    }
    ...
    private static boolean createPair(World sourceWorld, BlockPos sourcePortalPos, EnumFacing sourceSide, World destWorld, TeleportDestination destination, java.util.UUID gunId, boolean autoClose) {
        return createPair(sourceWorld, sourcePortalPos, sourceSide, destWorld, destination, gunId, autoClose, false, false);
    }

    private static boolean createPair(World sourceWorld, BlockPos sourcePortalPos, EnumFacing sourceSide, World destWorld, TeleportDestination destination, java.util.UUID gunId, boolean autoClose, boolean allowAirSource, boolean blueFluid) {
        if (allowAirSource ? !isValidAirSourcePlacement(sourceWorld, sourcePortalPos) : !isValidSourcePlacement(sourceWorld, sourcePortalPos)) return false;
