...
public class GuiAskCoords extends GuiScreen {
    ...
    @Override public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        boolean blueFluid = PortalGunItem.isBlueFluidActive(PortalGunItem.getGun(mc.player));
        drawCenteredString(fontRenderer, blueFluid ? "Custom Destination: X;Y;Z" : "Custom Destination: X;Y;Z [;DIMID|;DIRECTION|;DIRECTION;DIMID]", width / 2, height / 2 - 65, 0xFFFFFF);
        ...
        if (blueFluid) {
            drawCenteredString(fontRenderer, PortalGunItem.BLUE_DIMENSION_LIST_ERROR, width / 2, height / 2 - 90, 0xFF55AAFF);
        } else {
        ...
        try {
            ...
            if (parts.length == 4) {
                ...
                if (fourth.length() != 1 || "UDWENSudwens".indexOf(fourth.charAt(0)) < 0) {
                    if (blueFluid) return PortalGunItem.BLUE_DIMENSION_LIST_ERROR;
