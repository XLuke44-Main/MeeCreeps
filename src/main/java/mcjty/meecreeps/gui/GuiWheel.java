...
public class GuiWheel extends GuiScreen {
    ...
    @Override public void initGui() { super.initGui(); guiLeft = (width - WIDTH) / 2; guiTop = (height - HEIGHT) / 2; }
            ...
            if (lastSelected >= 0 && lastRing >= 0) {
                ...
                if (destinations.get(lastSelected) != null) {
                    MeeCreepsMessages.INSTANCE.sendToServer(new PacketSendServerCommand(MeeCreeps.MODID, CommandHandler.CMD_DELETE_DESTINATION,
                            TypedMap.builder().put(CommandHandler.PARAM_ID, encode(lastRing, lastSelected)).build()));
                }
            }
            return;
        }
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (existing != null && mouseButton == 0) {
            MeeCreepsMessages.INSTANCE.sendToServer(new PacketSendServerCommand(MeeCreeps.MODID, CommandHandler.CMD_SET_CURRENT,
                    TypedMap.builder().put(CommandHandler.PARAM_ID, encode(ring, q)).build()));
            closeThis();
            return;
        }

        // Creating a saved destination still uses the player's current location. Right-click
        // explicitly forces a floor/up-oriented destination before calculating its placement.
        EnumFacing destinationSide = mouseButton == 1 ? EnumFacing.UP : selectedSide;
        BlockPos bestPosition = TeleportationTools.findBestPosition(mc.world, selectedBlock, destinationSide);
        ...
        GuiAskName.destination = new TeleportDestination("", mc.world.provider.getDimension(), bestPosition, destinationSide);
        closeThis();
    ...
    private static int encode(int ring, int slot) { return ring * PortalGunItem.SLOTS_PER_RING + slot; }

    private void closeThis() { mc.displayGuiScreen(null); if (mc.currentScreen == null) mc.setIngameFocus(); }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        for (int ring = 0; ring < 3; ring++) {
            ...
            drawTexturedModalRect(x, guiTop, 0, 0, RING_WIDTH, HEIGHT);
            ...
            if (currentDestination >= ring * PortalGunItem.SLOTS_PER_RING
                ...
                drawSelectedSection(x, selected, 128);
            }
    ...
    private void drawIcons(int ringX, int ring) {
        List<TeleportDestination> destinations = PortalGunItem.getDestinations(PortalGunItem.getGun(Minecraft.getMinecraft().player), ring);
        int offset = 4;
        for (int i = 0; i < 8; i++) {
    ...
    private void drawTooltip(int ring, int q, int mouseX, int mouseY) {
        List<TeleportDestination> destinations = PortalGunItem.getDestinations(PortalGunItem.getGun(Minecraft.getMinecraft().player), ring);
        TeleportDestination destination = destinations.get(q);
        List<String> lines = new ArrayList<>();
        if (destination == null) {
            lines.add(TextFormatting.BLUE + "Left-Click: " + TextFormatting.WHITE + "save current location");
        } else {
            lines.add(TextFormatting.BLUE + "Left-Click: " + TextFormatting.WHITE + "select destination");
            lines.add(TextFormatting.RED + "Del: " + TextFormatting.WHITE + "remove destination");
        }
        lines.add(TextFormatting.AQUA + "Right-Click: " + TextFormatting.WHITE + "save current location (vertical)");
        lines.add(TextFormatting.GREEN + "Insert: " + TextFormatting.WHITE + "custom coordinates (temporary)");
        drawHoveringText(lines, mouseX, mouseY);
    }

    private void drawSelectedSection(int x, int q, int voffset) {
        mc.getTextureManager().bindTexture(hilight);
        int top = guiTop;
        // The logical destination slot order is rotated by four positions relative to
        // the wheel texture. This is the same mapping used by the original wheel.
        switch ((q - 4 + 8) % 8) {
            case 0: drawTexturedModalRect(x + 78, top, 0, voffset, 63, 63); break;
            case 1: drawTexturedModalRect(x + 107, top + 22, 64, voffset, 63, 63); break;
            case 2: drawTexturedModalRect(x + 107, top + 78, 128, voffset, 63, 63); break;
            case 3: drawTexturedModalRect(x + 78, top + 108, 192, voffset, 63, 63); break;
            case 4: drawTexturedModalRect(x + 23, top + 107, 0, voffset + 64, 63, 63); break;
            case 5: drawTexturedModalRect(x, top + 78, 64, voffset + 64, 63, 63); break;
            case 6: drawTexturedModalRect(x, top + 22, 128, voffset + 64, 63, 63); break;
            case 7: drawTexturedModalRect(x + 22, top, 192, voffset + 64, 63, 63); break;
        }
    }

    private int getSelectedSection(int cx, int cy) {
        double dist = Math.sqrt(cx * cx + cy * cy);
        if (dist < 37 || dist > 80) return -1;
        int q;
        if (cx >= 0 && cy < 0 && Math.abs(cx) < Math.abs(cy)) q = 0;
        else if (cx >= 0 && cy < 0) q = 1;
        else if (cx >= 0 && cy >= 0 && Math.abs(cx) >= Math.abs(cy)) q = 2;
        else if (cx >= 0 && cy >= 0) q = 3;
        else if (cx < 0 && cy >= 0 && Math.abs(cx) < Math.abs(cy)) q = 4;
        else if (cx < 0 && cy >= 0) q = 5;
        else if (cx < 0 && cy < 0 && Math.abs(cx) >= Math.abs(cy)) q = 6;
        else q = 7;
        return (q + 4) % 8;
    }
}
