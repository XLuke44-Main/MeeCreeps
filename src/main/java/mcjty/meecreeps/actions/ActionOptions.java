...
public class ActionOptions implements IActionContext {
    ...
    public ActionOptions(ByteBuf buf) {
        ...
        int targetSideIndex = buf.readByte();
        targetSide = targetSideIndex >= 0 && targetSideIndex < EnumFacing.VALUES.length ? EnumFacing.VALUES[targetSideIndex] : EnumFacing.UP;
        ...
        timeout = buf.readInt();
        int stageIndex = buf.readByte();
        stage = stageIndex >= 0 && stageIndex < Stage.values().length ? Stage.values()[stageIndex] : Stage.DONE;
        if (buf.readBoolean()) {
    ...
    public ActionOptions(NBTTagCompound tagCompound) {
        ...
        int targetSideIndex = tagCompound.getByte("targetSide");
        targetSide = targetSideIndex >= 0 && targetSideIndex < EnumFacing.VALUES.length ? EnumFacing.VALUES[targetSideIndex] : EnumFacing.UP;
        ...
        Stage loadedStage = Stage.getByCode(tagCompound.getString("stage"));
        stage = loadedStage == null ? Stage.DONE : loadedStage;
        ...
        if (timeout <= 0) {
            ...
            switch (stage) {
                ...
                case WAITING_FOR_PLAYER_INPUT:
                    ...
                    World serverWorld = DimensionManager.getWorld(0);
                    MinecraftServer server = serverWorld == null ? null : serverWorld.getMinecraftServer();
                    EntityPlayerMP player = (playerId == null || server == null) ? null : server.getPlayerList().getPlayerByUUID(playerId);
                    if (player == null) {
