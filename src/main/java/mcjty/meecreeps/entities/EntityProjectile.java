...
public class EntityProjectile extends EntityThrowable {
    ...
    private static final DataParameter<Boolean> BLUE_FLUID = EntityDataManager.createKey(EntityProjectile.class, DataSerializers.BOOLEAN);
    ...
    protected void entityInit() {
        super.entityInit();
        dataManager.register(BLUE_FLUID, false);
    }
    ...
    public void setBlueFluid(boolean blueFluid) {
        this.blueFluid = blueFluid;
        if (dataManager != null) dataManager.set(BLUE_FLUID, blueFluid);
    }
    public boolean isBlueFluid() {
        return dataManager != null ? dataManager.get(BLUE_FLUID) : blueFluid;
    }
        ...
        try {
            ...
            if (airPlacementEnabled && hasReachedAirPlacementDistance()) {
                ...
                if (airPosition.getY() >= 1 && airPosition.getY() <= 254
                    ...
                    if (player != null && TeleportationTools.makeAirPortalPair(player, airPosition, airSide, destination, gunId, autoClose)) {
                        setDead();
                        return;
                    }
                }
    ...
    protected void onImpact(RayTraceResult result) {
        ...
        try {
            if (result.typeOfHit == RayTraceResult.Type.BLOCK && destination != null && DimensionManager.isDimensionRegistered(destination.getDimension())) {
                ...
                if (player != null) {
                    TeleportationTools.makePortalPair(player, result.getBlockPos(), result.sideHit, destination, gunId, autoClose);
                } else if (!blueFluid || destination.getDimension() == world.provider.getDimension()) {
                    TeleportationTools.makePortalPair(world, result.getBlockPos(), result.sideHit, destination, blueFluid);
                }
            }
