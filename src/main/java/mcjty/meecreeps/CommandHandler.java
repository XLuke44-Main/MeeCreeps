...
public class CommandHandler {
    ...
    public static final String CMD_TOGGLE_AIR_PLACEMENT = "toggle_air_placement";
    ...
    public static void registerCommands() {
        ...
        McJtyLib.registerCommand(MeeCreeps.MODID, CMD_SET_CUSTOM_DESTINATION, (player, arguments) -> {
            ...
            if (destination != null && input != null && input.indexOf(';') < 0
                    && PortalGunItem.isIntradimensionalActive(heldItem)
                    && destination.getDimension() != player.getEntityWorld().provider.getDimension()) {
                player.sendMessage(new TextComponentString(PortalGunItem.INTRADIMENSIONAL_PLAYER_DIMENSION_ERROR)
        ...
        McJtyLib.registerCommand(MeeCreeps.MODID, CMD_TOGGLE_AIR_PLACEMENT, (player, arguments) -> {
            ItemStack heldItem = PortalGunItem.getGun(player);
            if (heldItem.isEmpty()) return false;
            boolean enabled = PortalGunItem.toggleAirPlacement(heldItem);
            player.sendMessage(new TextComponentString("Air Placement: " + (enabled ? "ENABLED" : "DISABLED")));
            return true;
        });
        McJtyLib.registerCommand(MeeCreeps.MODID, CMD_LIST_DIMENSIONS, (player, arguments) -> {
            ItemStack heldItem = PortalGunItem.getGun(player);
            if (!heldItem.isEmpty() && PortalGunItem.isIntradimensionalActive(heldItem)) {
                player.sendMessage(new TextComponentString(PortalGunItem.INTRADIMENSIONAL_DIMENSION_LIST_ERROR)
                        .setStyle(new Style().setColor(TextFormatting.BLUE)));
                return true;
            }
