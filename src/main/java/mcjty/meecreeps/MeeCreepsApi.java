...
public class MeeCreepsApi implements IMeeCreepsApi {
    ...
    public boolean spawnMeeCreep(String id, String furtherQuestionId, World world, BlockPos targetPos, EnumFacing targetSide, @Nullable EntityPlayerMP player, boolean doSound) {
        if (id == null || world == null || targetPos == null || targetSide == null) return false;

        MeeCreepActionType type = new MeeCreepActionType(id);
        Factory factory = getFactory(type);
        if (factory == null) return false;
        if (!factory.getFactory().isPossible(world, targetPos, targetSide)
                && !factory.getFactory().isPossibleSecondary(world, targetPos, targetSide)) {
            return false;
        }
        ...
        if (!ActionOptions.spawn(world, targetPos, targetSide, actionId, doSound)) {
            manager.cancelAction(player, actionId);
            return false;
        }
        manager.performAction(player, actionId, type, furtherQuestionId);
        return manager.getOptions(actionId) != null;
    }
