...
public class MeeCreepsAchievements {
    ...
    private static final ResourceLocation EXTRADIMENSIONAL_TRAVEL = new ResourceLocation(MeeCreeps.MODID, "extradimensional_travel");
    private static final ResourceLocation HYPERDIMENSIONAL_TRAVEL = new ResourceLocation(MeeCreeps.MODID, "hyperdimensional_travel");
    ...
    private static RefillState checkRefill(EntityPlayerMP player, ItemStack stack, RefillState state) {
        ...
        int maxCharge = CartridgeItem.getMaxCharge(CartridgeItem.getTransportSolutionType(stack));
        ...
        if (charge >= maxCharge && state.charge < maxCharge) {
            mcjty.meecreeps.items.TransportSolutionType type = CartridgeItem.getTransportSolutionType(stack);
            if (type == mcjty.meecreeps.items.TransportSolutionType.INTRADIMENSIONAL) {
                grant(player, LOCAL_TRAVEL, "refilled");
            } else if (type == mcjty.meecreeps.items.TransportSolutionType.EXTRADIMENSIONAL) {
                grant(player, EXTRADIMENSIONAL_TRAVEL, "refilled");
            } else if (type == mcjty.meecreeps.items.TransportSolutionType.HYPERDIMENSIONAL) {
                grant(player, HYPERDIMENSIONAL_TRAVEL, "refilled");
            } else {
